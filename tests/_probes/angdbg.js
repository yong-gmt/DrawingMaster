...
(async()=>{
  ...
  console.log(JSON.stringify(await p.evaluate(()=>{ const h=window.__hook();
    ...
    return (h.dimModels(pg)||[]).filter(m=>m.kind==='angular').map(m=>({
      deg:+(Math.abs(m.sweep)*180/Math.PI).toFixed(2), a0:+(m.a0*180/Math.PI).toFixed(1), sweep:+(m.sweep*180/Math.PI).toFixed(1), a0:+(m.a0*180/Math.PI).toFixed(1), sweep:+(m.sweep*180/Math.PI).toFixed(1),
