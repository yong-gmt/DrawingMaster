...
/* A value riding an arc leans with it: it stands square to the radius at the
   point it sits on, which is the tangent there, with its BASE towards the arc.
   It is not folded. Folding it to stay upright turned it over half way round, so
   its top faced the arc instead of its base - and since a value is placed by its
   baseline, the flipped number sank onto the very line it was meant to clear.
   The value keeps the one relationship to its arc wherever it is dragged. */
function dimArcTextRot(a){
  const t=a - Math.PI/2;                    /* the tangent, in degrees below */
  return Math.atan2(Math.sin(t), Math.cos(t))*180/Math.PI;
}
/* How far from the centre the value's BASELINE sits: the arc, then the same clear
   space every other dimension keeps, measured to the real ink - the rule
   dimTextPlace uses for a straight line. The gap is a fixed number of pixels at
   print density; dividing by the zoom instead made it whatever the zoom happened
   to be when the dimension was last redrawn. */
function dimArcTextR(m){
  const gap=(m.text.gapPx==null?DIM_TXT_GAP_PX:m.text.gapPx)/dimPxPerMM();
  return m.radius + gap + dimInk(m.text.h||2.5, dimValueText(m)).desc;
}
function dimAngularGeom(m){
  ...
  /* How far past each end the value sits, in radians. Measured from the MIDDLE of
     the arc and wrapped to a half turn either way, so the nearer end is the one
     it is past. Dividing the raw angle by the sweep did not wrap: a value dragged
     just past one end, where atan2 jumps from +180 to -180, read as being most of
     a turn past the OTHER end - and the arc ran round from there to meet it, all
     but closing into a circle. */
  const dir=(sw>=0)?1:-1, half=Math.abs(sw)/2;
  let b0=0, b1=0;
  if(ta!=null){
    const d=(ta-(a0+sw/2))*dir;
    const rel=Math.atan2(Math.sin(d), Math.cos(d));   /* + towards the far end */
    b1=Math.max(0, rel-half); b0=Math.max(0, -rel-half);
  }
  const tOut=Math.max(b0, b1);
  ...
  if(out){
    ...
    if(ta!=null){
      ...
      if(b0>0) past0=Math.max(stub, b0*r + halfText);
      if(b1>0) past1=Math.max(stub, b1*r + halfText);
    }
    const cap={s:true, e:false, h:m.line.arrow, w:m.line.arrowW};
    segs.push({role:'arcOut0', pts:sample(a0, a0-dir*past0/r), arrow:cap});
    segs.push({role:'arcOut1', pts:sample(a0+sw, a0+sw+dir*past1/r), arrow:cap});
  }
  ...
  /* The text is anchored by its baseline, and the baseline faces the arc, so the
     baseline sits one clear space out. Adding half the text height on top put the
     value further off its arc than any other dimension keeps its value. */
  const rt=(m.text.tr!=null)? m.text.tr : dimArcTextR(m);
